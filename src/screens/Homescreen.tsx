...
const VERSION_OF_NEW_FEATURE_RECOVERY = '1.2.7';
const VERSION_OF_NEW_FEATURE_SEND_BACKUP = '1.2.9';
const Homescreen: React.FC<HomescreenProps> = ({
  ...
  const [isNewFeatureRecoveryReady, setIsNewFeatureRecoveryReady] = useState(false);
  const [isNewFeatureSendBackupReady, setIsNewFeatureSendBackupReady] = useState(false);
  ...
  useEffect(() => {
    ...
    const isNewFeatureRecoveryReady =
      compareVersions(VERSION_OF_NEW_FEATURE_RECOVERY, latestSnapVersion) <= 0 &&
      compareVersions(VERSION_OF_NEW_FEATURE_RECOVERY, currentSnapVersion) <= 0;
    setIsNewFeatureRecoveryReady(isNewFeatureRecoveryReady);
    const isNewFeatureSendBackupReady =
      compareVersions(VERSION_OF_NEW_FEATURE_SEND_BACKUP, latestSnapVersion) <= 0 &&
      compareVersions(VERSION_OF_NEW_FEATURE_SEND_BACKUP, currentSnapVersion) <= 0;
    setIsNewFeatureSendBackupReady(isNewFeatureSendBackupReady);
  ...
  return (
    <div className="animate__animated animate__slideInUp animate__faster">
      <>
                  ...
                  <PopoverContent className="flex justify-center w-18 p-1 bg-[#111112] border-[#434E61] border-[1px]">
                    <div className="flex flex-col gap-2 bg-[#111112] text-white-primary p-2">
                      <div
                        ...
                        onClick={!isNewFeatureRecoveryReady ? undefined : onRePairing}>
                        <div
                          className={`flex rounded-full p-2 mr-2 ${
                            !isNewFeatureRecoveryReady ? 'opacity-50' : 'opacity-100'
                          }`}
                        ...
                        <div className="flex flex-col">
                          ...
                          {!isNewFeatureRecoveryReady && <span>(Update Snap to conitnue)</span>}
                        </div>
                      ...
                      <div
                        ...
                        <div
                          className="flex rounded-full p-1 mr-2"
                          style={{ background: '#23272E' }}>
                          <svg
                            width="32"
                            height="32"
                            ...
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <rect width="32" height="32" rx="16" fill="#23272E" />
                            <path
                              fillRule="evenodd"
                              clipRule="evenodd"
                              d="M10.5355 12.9299C11.279 10.9913 13.1603 9.125 16 9.125C19.0602 9.125 21.6269 11.316 22.1813 14.8037C22.9774 14.9227 23.8166 15.2239 24.5095 15.7538C25.3771 16.4173 26 17.4318 26 18.8125C26 20.167 25.4878 21.2139 24.6168 21.9073C23.7662 22.5845 22.6372 22.875 21.4688 22.875H11.3125C8.44729 22.875 6 20.8537 6 17.875C6 16.3649 6.5805 15.1616 7.5124 14.3115C8.33511 13.5611 9.40532 13.1084 10.5355 12.9299ZM16 10.375C13.6383 10.375 12.105 12.0173 11.5979 13.6821C11.5244 13.9234 11.3131 14.0968 11.0622 14.1219C9.99493 14.2286 9.03657 14.6132 8.35478 15.2351C7.68513 15.8459 7.25 16.7128 7.25 17.875C7.25 20.0526 9.02146 21.625 11.3125 21.625H21.4688C22.4487 21.625 23.2729 21.3795 23.8383 20.9294C24.3833 20.4955 24.75 19.8236 24.75 18.8125C24.75 17.8564 24.3377 17.196 23.7502 16.7468C23.1426 16.2821 22.3337 16.0378 21.5925 15.9992C21.284 15.9831 21.0336 15.7441 21.0031 15.4367C20.6841 12.224 18.4965 10.375 16 10.375Z"
                              fill="#F6F7F9"
                            />
                            <rect
                              width="10"
                              height="10"
                              rx="5"
                              transform="matrix(0 1 1 0 19 16)"
                              fill="#23272E"
                            />
                            <path
                              fillRule="evenodd"
                              clipRule="evenodd"
                              d="M20.4152 22.3565C20.2076 21.8714 20.1009 21.3493 20.1016 20.8217C20.1017 18.6686 21.8484 16.9219 24.0016 16.9219C26.1548 16.9219 27.9016 18.6687 27.9016 20.8219V20.8219C27.9015 21.6285 27.6515 22.4152 27.1859 23.0739C26.7203 23.7325 26.062 24.2306 25.3017 24.4997C25.1455 24.555 24.974 24.4732 24.9188 24.317C24.8635 24.1608 24.9453 23.9894 25.1015 23.9341C25.7449 23.7064 26.302 23.2849 26.6959 22.7275C27.0899 22.1702 27.3015 21.5044 27.3016 20.8219L20.7016 20.8217C20.7016 20.8218 20.7016 20.8218 20.7016 20.8219V20.8221L20.7016 20.8222C20.701 21.2685 20.7913 21.7102 20.9668 22.1204C21.1414 22.5284 21.3967 22.8968 21.7173 23.2035L21.9351 23.3906L22.8895 22.4362C23.0785 22.2472 23.4016 22.3811 23.4016 22.6485V24.7219C23.4016 24.8014 23.37 24.8777 23.3137 24.934C23.2574 24.9903 23.1811 25.0219 23.1016 25.0219H21.0282C20.7608 25.0219 20.6266 24.6988 20.8159 24.5098L21.5096 23.8161L21.3145 23.6484L21.309 23.6431C20.9269 23.2793 20.6228 22.8415 20.4152 22.3565ZM20.7016 20.8217L27.3016 20.8219C27.3016 19 25.8234 17.5219 24.0016 17.5219C22.1798 17.5219 20.7017 19 20.7016 20.8217Z"
                              fill="#F6F7F9"
                            />
                          </svg>
                        </div>
                        <div className="flex flex-col">
                          <span
                            className={`${
                              !isNewFeatureSendBackupReady ? 'opacity-50' : 'opacity-100'
                            }`}>
                            Resend Backup
                          </span>
                          {!isNewFeatureSendBackupReady && <span>(Update Snap to conitnue)</span>}
                        </div>
                        <div className="flex-1"></div>
                      </div>
        ...
        <Dialog open={showEnryptedBackupSentDailog} onOpenChange={setShowEnryptedBackupSentDailog}>
          <DialogContent
            className="flex flex-col items-center justify-center py-14 px-14 h-auto bg-[#121212] border-none outline-none max-w-[634px] min-w-[450px]"
            onInteractOutside={(e) => {
              e.preventDefault();
            }}>
            <img src="/v2/checkmarkCircleLight.png" alt="checkmark-circle" width={100} />
            <div
              className="mt-2 text-center text-white-primary"
              style={{ fontSize: 16, fontWeight: 500 }}>
              Encrypted backup sent
            </div>
            <div
              className="mt-2 text-center text-white-primary"
              style={{ fontSize: 14, fontWeight: 400 }}>
              Now you can safely backup your account to your preferred choice of storage on your
              Silent Shard Phone app.
            </div>
            <Button
              className="max-sm:p-8 w-1/2 mt-2 h-[48px] bg-indigo-primary hover:bg-indigo-hover active:bg-indigo-active text-white-primary btn-lg whitespace-normal "
              onClick={async () => {
                setShowEnryptedBackupSentDailog(false);
              }}>
              Continue
            </Button>
          </DialogContent>
